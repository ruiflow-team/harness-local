...
```text
harness-local-v0.26.0-beta.zip
```
...
不可以。当前是本地 beta 工具。任何对外发布、官网/公众号内容或公开承诺，都应先经过项目负责人的人工确认。
...
- `./harness doctor`：输出 `diagnosis` 三层体检结果：`must_fix` / `should_fix` / `can_ignore`。
- `./harness doctor-report`：输出 Markdown 体检报告，便于本地自查和反馈前人工检查。
- `./harness feedback-bundle`：生成本地脱敏反馈包，目录为 `dist/evidence/FEEDBACK-BUNDLE/`，并包含 `DOCTOR-REPORT.md`；提交公开 issue 前请人工检查 `README.md`、`FEEDBACK-SUMMARY.md`、`DOCTOR-REPORT.md`、`doctor.json`，确认没有密钥、私有源码、原始反馈 JSONL 或客户数据。
